...
const App=()=> {
  ...
  return (
    <div className="App">
      ...
      <div><Nav></Nav></div>
      <div className="container">{dataPoke.map((val,num)=> <CardC id={val.id} name={val.name} imageF={val.sprites.other.home.front_default}  stat1={val.stats[0].base_stat} stat2={val.stats[1].base_stat} stat3={val.stats[2].base_stat} stat4={val.stats[5].base_stat} exp={val.base_experience} weight={val.weight} type={val.types.length === 1 ? <div className="type-pok"><p className={`type type-${val.types[0].type.name}`}>{capitalizeFirstLetter(val.types[0].type.name)}</p></div> : <div className="type-pok"><p className={`type type-${val.types[0].type.name}`}>{capitalizeFirstLetter(val.types[0].type.name)}</p><p p className={`type type-${val.types[1].type.name}`}>{capitalizeFirstLetter(val.types[1].type.name)}</p></div>} key={num} 
      ...
      <div className="Pag">
        <a className="Pag-element">Prev</a>
        <a className="Pag-element">Next</a>  
      </div>
      <div>
        <Footer/>
      </div>
